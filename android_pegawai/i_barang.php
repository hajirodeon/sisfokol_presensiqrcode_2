...
if ((isset($_GET['aksi']) && $_GET['aksi'] == 'pinjam'))
	...
	if (empty($tku))
		...
		$qcc = mysqli_query($koneksi, "SELECT * FROM item_pinjam ".
								"WHERE item_kd = '$ku_kd' ".
								"AND status = 'PINJAM' ".
		...
		if (!empty($tcc))
			...
			if (!empty($tcc))
				...
				{
				//insert
				mysqli_query($koneksi, "INSERT INTO item_pinjam(kd, orang_kd, item_kd, postdate, ".
...
if ((isset($_GET['aksi']) && $_GET['aksi'] == 'kembali'))
	...
	if (empty($tku))
		...
		if (!empty($tcc))
			{
			//update
			mysqli_query($koneksi, "UPDATE item_pinjam SET status = 'KEMBALI', ".
