			    ...
			    if ($i > 1) 
			    	{
					  ...
					  $i_qrcode = "$kdepan$i_kode";
						...
						if (!empty($tcc))
							{
							mysqli_query($koneksi, "UPDATE m_item SET nama = '$i_nama' ".
											"WHERE kode = '$i_kode'");
							}
							...
							{
							//insert
							mysqli_query($koneksi, "INSERT INTO m_item(kd, kode, nama, qrcode, postdate) VALUES ".
											"('$i_xyz', '$i_kode', '$i_nama', '$i_qrcode', '$today')");
							}
...
if ($_POST['btnSMP'])
	...
	if ((empty($e_nip)) OR (empty($e_nama)))
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE m_item SET kode = '$e_nip', ".
							"nama = '$e_nama' ".
							"WHERE kd = '$kd'");

			//update
			mysqli_query($koneksi, "UPDATE m_item SET filex1 = '$namabaru', ".
							"postdate = '$today' ".
							"WHERE kd = '$kd'");
		...
		if ($s == "baru")
			...
			if ($tcc != 0)
				...
				{
				mysqli_query($koneksi, "INSERT INTO m_item(kd, kode, nama, qrcode, postdate) VALUES ".
								"('$kd', '$e_nip', '$e_nama', '$e_qrcode', '$today')");
  ...
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.0/jquery.min.js"></script>  
   ...
   <hr>';
	...
	$(document).ready(function(){
	       ...
	       $('#image-holder').load("<?php echo $sumber;?>/adm/item/i_item.php?aksi=lihat&kd=<?php echo $kd;?>");
	    ...
	    $('#upload_image').on('change', function(event){
	     event.preventDefault();
			...
			$('#loading').show();
		     ...
		     $.ajax({
		      url:"<?php echo $sumber;?>/adm/item/i_item_upload.php?kd=<?php echo $kd;?>",
		      method:"POST",
		      ...
		      success:function(data){
				$('#loading').hide();
		       $('#preview').load("<?php echo $sumber;?>/adm/item/i_item.php?aksi=lihat&kd=<?php echo $kd;?>");
	...
	if ($count != 0)
		...
		do 
			...
			if (empty($i_kode))
				...
				//update...
				mysqli_query($koneksi, "UPDATE m_item SET kode = '$kodenya', ".
								"qrcode = '$kodenya' ".
								"WHERE kd = '$i_kd'");
				}
			...
			<hr>
			<a href="item_qrcode.php?kd='.$i_kd.'" target="_blank" class="btn btn-danger">PRINT QRCODE >></a>
